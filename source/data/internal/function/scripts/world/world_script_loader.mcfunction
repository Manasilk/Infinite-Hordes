...
execute if score #world world_tick matches 0 as @e[type=#internal:area_trigger,tag=!ENTITY_FLAG_QUEST_TRIGGER,tag=!ENTITY_FLAG_EXTRA_INSTANCE_BIND] at @s run function internal:scripts/world/area_trigger_script_loader
...
execute if score #world world_tick matches 0.. as @e[type=#internal:area_trigger,tag=ENTITY_FLAG_EXTRA_INSTANCE_BIND] at @s run function internal:scripts/instances/instance_script_loader
